...
if [ -z "$SOC" ] || [ -z "$BOARD" ]; then
	...
	echo "---Attention!! redmi_ax6000_512 for 512MB SPI-NAND only!!---"
	echo "eg: SOC=mt7986 BOARD=redmi_ax6000_512 $0"	
	exit 1
fi
...
if [ ! -f $ATF_DIR/configs/$ATF_CFG ]; then
	...
	if [ -f "$ATF_DIR/build/${SOC}/release/fip.bin" ]; then
		...
		BL2_NAME="${SOC}_${BOARD}-bl2.bin"
		if [ "${BOARD}" = "redmi_ax6000_512" ]; then
			cp -f $ATF_DIR/build/${SOC}/release/bl2.img output/${BL2_NAME}
